...
export const paymongoWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    if (webhookSecret) {
      ...
      if ((parts["li"] ?? parts["te"]) !== expected) { res.status(401).send("Invalid signature"); return; }
    }

    if (req.body?.data?.attributes?.type !== "link.payment.paid") { res.status(200).send("Ignored"); return; }

    const linkId: string | undefined = req.body?.data?.attributes?.data?.id;
    if (!linkId) { res.status(400).send("Missing link id"); return; }

    const snap = await db.collection("appointments").where("paymongoPaymentIntentId", "==", linkId).limit(1).get();
    if (snap.empty) { res.status(200).send("No match"); return; }
