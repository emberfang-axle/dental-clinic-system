...
/** Order matters: last account stays signed in — must be **doctor** so Firestore cleanup can delete stale user docs. */
...
export function SetupPage({ navigate }: { navigate: (p: string) => void }) {
  ...
  async function cleanOrphans() {
    ...
    setLog([]);
    try {
      const me = await getDocTyped<User>("users", auth.currentUser?.uid ?? "");
      if (!me || (me.role !== "doctor" && me.role !== "admin")) {
        append("❌ Sign in as the clinic doctor or admin first, then try again.");
        setCleaning(false);
        return;
      }
  ...
  async function run() {
    ...
    // Remove stale/duplicate Firestore user docs (requires doctor session — last ACCOUNTS entry is doctor)
    try {
      const allUsers = await listCollection<User>("users");
      ...
      if (msg.includes("permission") || msg.includes("Permission")) {
        append("⚠️ Cleanup skipped: sign in as clinic doctor and run setup again, or remove stale users in Firebase Console.");
      } else {
        append(`⚠️ Cleanup error: ${msg}`);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-ink-950 text-gold-50 flex items-center justify-center p-6">
      <div className="max-w-lg w-full glass-strong rounded-2xl p-8 shadow-luxe space-y-6">
        ...
        <p className="text-xs text-amber-300/80 border border-amber-500/20 rounded-lg p-3">
          Production: set <code className="text-gold-300">VITE_SETUP_SECRET</code> in your env and open{" "}
          <code className="text-gold-300">/?setup_key=YOUR_SECRET#/setup</code>, or sign in as admin/doctor.
        </p>
