...
import { StatGrid, TodaySchedule, AlertBanner, WeeklyMiniCalendar, RevenueSparkline, LatestAnnouncement, QuickActions, TodaySummaryWidget, CashSalesReport } from "../../components/ui/DashboardWidgets";
...
import { CoDoctorOverview } from "./CoDoctorOverview";

export function DoctorDashboard({ navigate }: { navigate: (p: string) => void }) {
  const { user } = useStore();
  const [tab, setTab] = useState("overview");
  if (!user) return null;
  if (user.role !== "doctor" && user.role !== "co-doctor" && user.role !== "admin") {
    return null;
  }

  const tabs =
    user.role === "co-doctor" ? DASHBOARD_TABS["co-doctor"] : DASHBOARD_TABS.doctor;

  return (
    <DashboardLayout user={user} tabs={tabs} activeTab={tab} onTabChange={setTab} navigate={navigate}>
      {tab === "overview"      && (user.role === "co-doctor"
        ? <CoDoctorOverview onTabChange={setTab} />
        : <DoctorOverview onTabChange={setTab} />)}
      {tab === "announcements" && <AnnouncementsPage />}
      {tab === "appointments"  && <AppointmentsList role="doctor" />}
      {tab === "audit"         && user.role !== "co-doctor" && <AuditLogsPage />}
      {tab === "clinical"      && <ClinicalRecords />}
      {tab === "feedback"      && user.role !== "co-doctor" && <DoctorFeedbackView />}
      {tab === "notifications" && <NotificationsCenter />}
      {tab === "payments"      && user.role !== "co-doctor" && <PaymentsPage role="doctor" />}
      {tab === "reports"       && user.role !== "co-doctor" && <ReportsPage />}
      {tab === "schedule"      && user.role !== "co-doctor" && <ScheduleRules />}
      {tab === "services"      && user.role !== "co-doctor" && <ServicesPage role="doctor" />}
      {tab === "staff"         && user.role !== "co-doctor" && <StaffAccounts />}
    ...
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <CashSalesReport appointments={appointments} onTabChange={onTabChange} />
        </div>
