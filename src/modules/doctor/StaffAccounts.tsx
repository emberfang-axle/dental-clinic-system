...
import { Badge, Button, Card, ConfirmDialog, Input, Label, PasswordInput, Select } from "../../components/ui";
...
export function StaffAccounts() {
  ...
  return (
    <div className="space-y-6">
                ...
                <div key={staff.id} className="rounded-2xl border border-gold-500/15 bg-ink-900/55 p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        type="button"
                        onClick={() => setRemoveTarget({ id: staff.id, name: staff.name, role: "staff" })}
            ...
            <div className="space-y-3">
              {coDoctorUsers.map((cd) => (
                <div key={cd.id} className="rounded-2xl border border-purple-500/20 bg-ink-900/55 p-4 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => setRemoveTarget({ id: cd.id, name: cd.name, role: "co-doctor" })}
                    className="text-xs text-red-400/70 hover:text-red-400 transition px-2 py-1 rounded border border-red-500/20 hover:border-red-500/50"
                  >
                    Remove
                  </button>
        ...
        <Card>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              if (!isValidEmail(newStaff.email)) { setCreateError("Enter a valid email (any provider, not only Gmail)."); return; }
      ...
      <ConfirmDialog
        open={!!removeTarget}
        title={`Remove ${removeTarget?.role === "co-doctor" ? "co-doctor" : "staff member"}?`}
        message={`Remove ${removeTarget?.name} from the system? They will no longer be able to sign in.`}
        confirmLabel="Remove"
        danger
        onCancel={() => setRemoveTarget(null)}
        onConfirm={async () => {
          if (removeTarget) await settingsService.removeStaff(removeTarget.id);
