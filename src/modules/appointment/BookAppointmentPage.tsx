 ...
 * 1. Select Service  2. Choose Doctor  3. Schedule  4. Payment & Confirm
...
import { formatDurationMinutes, estimateBookingMinutes, isOralConsultation } from "../../shared/serviceCatalog";
import { ServicePickerStep, serviceRequiresConsultation } from "./ServicePickerStep";
...
export function BookAppointmentPage({ navigate }: { navigate: (p: string) => void }) {
  ...
  if (services.length === 0) {
    return (
      <Centered navigate={navigate}>
        <div className="max-w-md w-full glass-strong rounded-2xl p-10 text-center shadow-luxe">
          <p className="text-gold-100/70 text-sm">Clinic services are still loading. Please refresh or try again shortly.</p>
          <Button className="mt-6" onClick={() => navigate(ROUTES.home)}>Back to Home</Button>
        </div>
      </Centered>
    );
  }
  ...
  const consultationService = services.find((s) => isOralConsultation(s.name));
  ...
  function toggleService(s: (typeof services)[0]) {
    if (serviceRequiresConsultation(s) && !hasConsultation) {
  ...
  function bookConsultationOnly() {
    const consult = services.find((s) => isOralConsultation(s.name));
    if (consult) {
      setSelectedServiceIds([consult.id]);
  ...
  function next() {
    ...
    if (step === 1) {
      if (selectedServiceIds.length === 0) {
        setBookingError("Please select at least one service to continue.");
        return;
      }
      const blocked = selectedServices.some(
        (s) => serviceRequiresConsultation(s) && !hasConsultation,
      );
      if (blocked) {
        setBookingError("One or more selected services require an Oral Consultation first.");
        return;
      }
    }
  ...
  async function confirm() {
    ...
    if (!time || !primaryService) return;
    if (appointmentsService.isSlotTaken(date, time, doctor)) {
      setBookingError("This time slot was just taken for your dentist. Please pick another.");
      return;
    }
    ...
    return (
      <Centered navigate={navigate}>
        <div className="max-w-lg w-full glass-strong rounded-2xl p-10 text-center shadow-luxe fade-up">
          ...
          <p className="text-[10px] uppercase tracking-[0.4em] text-amber-400 font-semibold mb-3">Pending Confirmation</p>
          <h2 className="font-serif text-4xl text-gold-shine font-light">
            Request <span className="font-script italic">Submitted!</span>
          </h2>
          <p className="text-gold-100/70 mt-5 leading-relaxed">
            ...
            <span className="text-gold-300 font-medium">{formatTime12h(time)}</span> was received.
          </p>
          <p className="text-sm text-gold-100/55 mt-3">
            Status: <strong className="text-amber-300/90">Pending</strong> — staff will confirm before your visit.
            You will get a notification when confirmed.
          </p>
        ...
        <div className="rounded-2xl glass-strong p-4 sm:p-8 shadow-luxe fade-up">
          {step === 1 && (
            <ServicePickerStep
              ...
              onSetSelection={setSelectedServiceIds}
              onConsultRequired={(name) => {
                setPendingSurgicalName(name);
                setShowConsultationModal(true);
              }}
              onBookConsultation={bookConsultationOnly}
            />
          )}
          ...
          {step === 4 && (
            <div>
              ...
              <div className="space-y-3 text-sm">
                ...
                <Row
                  label="Est. duration"
                  value={formatDurationMinutes(estimateBookingMinutes(selectedServices))}
                />
                ...
                />
                <Row label="Payment" value="Cash at clinic (after treatment)" />
              </div>
            </div>
          )}
