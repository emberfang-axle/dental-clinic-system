...
import type { Appointment, AppointmentStatus, PaymentMethod, PaymentStatus, Role } from "../../shared/types";
...
export function AppointmentsList({ role, patientOnly }: { role: Role | "admin"; patientOnly?: boolean }) {
  ...
  list = list.sort(sortAppointmentsLatestFirst);
...
function AppointmentDetail({ a, role, isSelf, actor }: { a: Appointment; role: Role | "admin"; isSelf: boolean; actor: string }) {
  ...
  async function confirmReschedule() {
    ...
    if (appointmentsService.isSlotTaken(newDate, newTime, a.doctor, a.id)) {
      setRescheduleError("This time slot is no longer available for your dentist.");
      return;
    }
    setSaving(true);
  ...
  return (
    <div className="space-y-3">
      ...
      {role === "patient" && isSelf && a.status !== "cancelled" && (
...
export function PaymentBadge({ status, method }: { status: PaymentStatus; method: PaymentMethod }) {
  return (
    <div className="flex flex-col items-start gap-0.5">
      ...
      <span className="text-[10px] text-gold-100/45 capitalize">{method === "gcash" ? "GCash" : "Cash"}</span>
    </div>
  );
}
