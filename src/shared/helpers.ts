...
/** Newest bookings first (patient + staff lists). Uses createdAt from Firestore. */
/** Doctor saved clinical / progress notes on this visit. */
export function hasTreatmentRecord(a: Appointment): boolean {
  return Boolean(
    a.progressNoteAt ||
      a.diagnosis?.trim() ||
      a.treatmentPlan?.trim() ||
      a.notes?.trim() ||
      a.complaint?.trim(),
  );
}

export function treatmentNotesPreview(a: Appointment): string {
  const parts: string[] = [];
  if (a.complaint?.trim()) parts.push(`Complaint: ${a.complaint.trim()}`);
  if (a.diagnosis?.trim()) parts.push(`Diagnosis: ${a.diagnosis.trim()}`);
  if (a.treatmentPlan?.trim()) parts.push(`Plan: ${a.treatmentPlan.trim()}`);
  if (a.notes?.trim()) parts.push(`Notes: ${a.notes.trim()}`);
  if (a.prescription?.trim()) parts.push(`Rx: ${a.prescription.trim()}`);
  return parts.join(" · ") || "No treatment notes yet — ask the dentist to save notes in Clinical Records.";
}

export function sortAppointmentsLatestFirst(a: Appointment, b: Appointment): number {
  const ka = a.createdAt ?? `${a.date}T${a.time}`;
  const kb = b.createdAt ?? `${b.date}T${b.time}`;
...
export function canAccessRoute(path: string, role: Role | undefined): boolean {
  ...
  if (path === ROUTES.doctorDashboard)    return role === "doctor" || role === "co-doctor";
  if (path === ROUTES.coDoctorDashboard)  return role === "co-doctor" || role === "doctor";
