/**
 * FDI tooth chart (odontogram) — click teeth to mark conditions.
...
export function ToothChart({ value, onChange, readOnly = false }: ToothChartProps) {
  const [activeCondition, setActiveCondition] = useState<ToothCondition>("Extraction");

  function getCondition(tooth: number): ToothCondition {
    return (value[String(tooth)] as ToothCondition) || "Healthy";
  }

  function markTooth(tooth: number) {
    if (readOnly) return;
    onChange?.({ ...value, [String(tooth)]: activeCondition });
  }
  ...
  function ToothButton({ tooth }: { tooth: number }) {
    ...
    return (
      <button
        ...
        disabled={readOnly}
        title={`${tooth}: ${cond}`}
        className={`w-9 h-9 rounded-lg border text-[10px] font-bold transition ${color} ${readOnly ? "cursor-default" : "hover:brightness-125 ring-offset-ink-950 focus-visible:ring-2 focus-visible:ring-gold-400"}`}
      >
        {tooth}
      </button>
    );
  ...
  const marked = Object.entries(value).filter(([, v]) => v && v !== "Healthy");

  return (
    <div className="space-y-4">
      {!readOnly && (
        <div className="rounded-xl border border-gold-500/20 bg-ink-900/50 p-4">
          <p className="text-xs text-gold-100/60 mb-2">
            Select a condition below, then click teeth on the chart (same as marking on paper records).
          </p>
          <div className="flex flex-wrap gap-2">
            {TOOTH_CONDITIONS.map((c) => (
              <button
                ...
                className={`text-[10px] px-2.5 py-1 rounded border transition ${CONDITION_COLORS[c]} ${
                  activeCondition === c ? "ring-2 ring-gold-400" : "opacity-70 hover:opacity-100"
                }`}
              >
                {c}
              </button>
      ...
      {marked.length > 0 && (
        <div className="rounded-lg border border-gold-500/15 bg-ink-900/40 p-3">
          <p className="text-[10px] uppercase tracking-wider text-gold-300/55 mb-2">Marked teeth</p>
          <p className="text-xs text-gold-100/70">
            {marked.map(([t, c]) => `#${t}: ${c}`).join(" · ")}
          </p>
        </div>
      )}
    </div>
