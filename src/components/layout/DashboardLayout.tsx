...
export function DashboardLayout({ user, tabs, activeTab, onTabChange, navigate, headerAction, children }: DashboardLayoutProps) {
  ...
  function openNotifications() {
    const notifTab = tabs.find((t) => t.id === "notifications");
    onTabChange(notifTab ? "notifications" : "profile");
  }
  ...
  return (
    <div className="min-h-screen bg-ink-950 flex relative overflow-hidden">
      ...
      {/* Desktop sidebar — text only, no icons */}
...
/** Desktop sidebar: text labels only, no icons */
function DesktopSidebar({ user, tabs, activeTab, onTabChange, navigate }: {
  user: User; tabs: DashboardTab[]; activeTab: string;
  onTabChange: (id: string) => void; navigate: (p: string) => void;
}) {
  return (
    <>
      <div className="p-4 border-b border-gold-soft">
        <button onClick={() => navigate(ROUTES.home)} className="hover:opacity-80 transition">
          <Logo size={32} />
        </button>
      </div>
      ...
      <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto overscroll-contain">
        {tabs.map((t) => (
          ...
          >
            {t.label}
            {activeTab === t.id && <span className="float-right w-1 h-4 rounded-full bg-gold-400 mt-0.5" />}
          </button>
        ))}
      </nav>
      ...
      <div className="p-2 border-t border-gold-soft space-y-1">
        ...
        <Button variant="ghost" className="w-full justify-start text-xs" onClick={() => { void authService.logout(); navigate(ROUTES.home); }}>
