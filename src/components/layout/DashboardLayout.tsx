  ...
  if (user.avatarUrl) {
    return (
      <img
        src={user.avatarUrl}
        alt=""
        className={`${className} rounded-full object-cover border border-gold-500/30 shrink-0`}
      />
    );
  }
  return (
    <span className={`${className} rounded-full bg-gold-gradient flex items-center justify-center text-ink-950 font-bold text-xs shrink-0`}>
      {initials(user.name)}
    </span>
  );
}
...
export function DashboardLayout({ user, tabs, activeTab, onTabChange, navigate, headerAction, children }: DashboardLayoutProps) {
  ...
  return (
    <div className="h-screen overflow-hidden bg-ink-950 flex relative">
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 z-20 w-60 h-screen border-r border-gold-soft bg-ink-900/95 backdrop-blur-xl flex-col shrink-0">
        <DesktopSidebar user={user} tabs={tabs} activeTab={activeTab} onTabChange={onTabChange} navigate={navigate} unread={unread} onRequestLogout={() => setConfirmLogout(true)} />
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10 lg:ml-60">
        <header className="min-h-16 border-b border-gold-soft px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-3 bg-ink-950/60 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="no-min hidden lg:flex items-center gap-2 px-2 py-1.5 rounded-full border border-gold-500/25 bg-ink-900/70 hover:bg-gold-500/10 hover:border-gold-400/50 transition"
              aria-label="Open profile"
              title={shortRole(user.role)}
            >
      ...
      <ConfirmDialog
        open={confirmLogout}
        title="Log out?"
        message={`Sign out as ${user.name}? You will need to sign in again to access your dashboard.`}
...
function DesktopSidebar({ user, tabs, activeTab, onTabChange, navigate, unread, onRequestLogout }: {
